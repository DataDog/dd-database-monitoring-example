...
# Pre-render the Postgres integration config as YAML so the user_data heredoc
...
locals {
  postgres_check_yaml = yamlencode({
    init_config = {}
    instances = [{
      dbm      = true
...
# Open Postgres ingress on the database's security group from the EC2 host SG.
...
resource "aws_security_group_rule" "db_ingress_from_agent" {
  type                     = "ingress"
  description              = "Datadog Agent EC2 host to Postgres"
...
# Single EC2 host running the Datadog Agent in Docker. The Postgres check is
# configured via a conf.yaml dropped into /etc/datadog-agent/conf.d/postgres.d
...
resource "aws_instance" "agent" {
  ...
  user_data = templatefile("${path.module}/user_data.sh.tpl", {
    postgres_check_yaml = local.postgres_check_yaml
